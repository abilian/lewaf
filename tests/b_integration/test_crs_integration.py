    ...
    for rule_file in [method_file, sqli_file, xss_file]:
        if rule_file.exists():
            ...
            if "911" in rule_file.name:
                assert "id:911" in content, "METHOD rules should have 911xxx IDs"
            elif "942" in rule_file.name:
                assert "id:942" in content, "SQLI rules should have 942xxx IDs"
            elif "941" in rule_file.name:
                assert "id:941" in content, "XSS rules should have 941xxx IDs"

            # Should have phase declarations
            assert "phase:" in content, (
                f"Rules should specify phases in {rule_file.name}"
            )

            # Should have OWASP_CRS tags
            assert "OWASP_CRS" in content, (
                f"Rules should have OWASP_CRS tag in {rule_file.name}"
            )
...
def test_crs_paranoia_levels():
    ...
    assert len(paranoia_markers) > 10, (
        f"Expected multiple paranoia level markers, found {len(paranoia_markers)}"
    )

    # Should have levels 0-4
    content_str = "\n".join(paranoia_markers)
    for level in [0, 1, 2, 3]:
        assert f"Level {level}" in content_str, (
            f"Should have paranoia level {level} markers"
        )
...
def test_crs_data_file_references():
    ...
    for expected in expected_files:
        if expected in data_file_names:
            assert expected in referenced_files, (
                f"Expected reference to {expected} in CRS rules"
            )
...
def test_crs_rule_metadata():
    ...
    assert len(found_ranges) >= 2, (
        f"Should find rules from multiple ranges, found: {found_ranges}"
    )
