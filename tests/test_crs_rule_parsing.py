...
def test_crs_file_format(crs_tester):
    ...
    for file_path in files:
        ...
        assert "Copyright" in content, \
            f"File {file_path.name} missing copyright notice"
    ...
    for file_path in files:
        ...
        for rule, line_num in rules:
            ...
            if operator.startswith('@'):
                op_name = operator[1:].split()[0]  # Get operator name
                # Verify it's a known operator type
                known_operators = [
                    'lt', 'le', 'gt', 'ge', 'eq', 'within', 'rx', 'contains',
                    'beginswith', 'endswith', 'detectsqli', 'detectxss',
                    'ipmatch', 'validatebyterange', 'validateutf8encoding',
                    'pm', 'pmf', 'pmfromfile', 'strmatch', 'streq'
                ]
...
def test_common_crs_variables(crs_tester):
    ...
    expected_variables = [
        ...
        'ARGS', 'ARGS_NAMES', 'REQUEST_COOKIES', 'REQUEST_FILENAME',
        'RESPONSE_HEADERS', 'RESPONSE_BODY', 'TX', 'FILES'
    ]
...
def test_crs_transformations_coverage(crs_tester):
    ...
    supported_transformations = {
        'lowercase', 'uppercase', 'length', 'trim', 
        'compresswhitespace', 'removewhitespace',
        'urldecode', 'urldecodeuni', 'htmlentitydecode',
        'jsdecode', 'cssjsdecode', 'base64decode', 'hexdecode',
        'md5', 'sha1', 'sha256', 'normalizepath',
        'removenulls', 'removenullbytes', 'replacecomments',
        'replacewhitespace'
    }
    ...
    if used_transformations:
        ...
        coverage = len(used_transformations & supported_transformations) / len(used_transformations)
        assert coverage >= 0.5, f"Only support {coverage:.1%} of transformations used in CRS"
...
def test_rule_parsing_with_seclang_parser(crs_tester):
    ...
    if total_tested > 0:
        ...
        print(f"Parser success rate: {success_rate:.1%} ({parsed_count}/{total_tested})")
